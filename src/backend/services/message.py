...
from .schemas import MessageGet, MessagePatch, MessagePost
...
class MessageService:
    ...
    async def _get_message_orm_by_id(self, message_id: int) -> Message:
        stmt = select(Message).where(Message.id == message_id)
        result = await self.db.execute(stmt)
    ...
    async def update_message(
        ...
        message = await self._get_message_orm_by_id(message_id)

        update_data = message_data.model_dump(exclude_unset=True, exclude={"id"})
